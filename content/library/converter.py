...
def latex_to_markdown(latex_text):
    def convert_figures(match):
        ...
        return f'<span class="annotation__text" data-annotation="{caption}">\n<img src="media/{image_path}" id="{label}" />\n</span>'

    latex_text = re.sub(
        r"\\begin{figure}(?:\[.*?\])?.*?\\includegraphics(?:\[.*?\])?{.*?}.*?\\caption{.*?}.*?\\label{.*?}.*?\\end{figure}",
        lambda m: convert_figures(m),
        latex_text,
        flags=re.DOTALL,
    )

    def convert_include_image(match, image_dir="media"):
        content = match.group(0)
        image_match = re.search(r"\\includegraphics(?:\[.*?\])?{(.+?)}", content)
        ...
        image_path = image_match.group(1).strip()
        return f'<img src="{image_dir}/{image_path}" />'

    latex_text = re.sub(
        r"\\includegraphics(?:\[.*?\])?{.*?}",
        lambda m: convert_include_image(m, image_dir="media"),
        latex_text,
        flags=re.DOTALL,
    )
    ...
    latex_text = re.sub(
        r"\\begin{document}(.+?)\\end{document}", r"\1", latex_text, flags=re.DOTALL
    )
    ...
    latex_text = re.sub(
        r"\\usepackage(\[.+?\])?{((.+?))}", r"[//]: # (usepackage: \1, \2)", latex_text
    )
    ...
    latex_text = re.sub(
        r"\\begin{(theorem|lemma|corollary|proposition|definition|example|maintheorem)}(?:\[(.*?)\])?\s*\n?(.*?)\n?\\end{\1}",
        lambda m: f'<div class="{m.group(1)}-box" data-title="{m.group(1).capitalize()}"'
        + (f' data-label="{m.group(2)}"' if m.group(2) else "")
        + ">\n"
        + "\n".join(line.lstrip() for line in m.group(3).splitlines())
        + "\n</div>",
        latex_text,
        flags=re.DOTALL,
    )
    ...
    latex_text = re.sub(
        ...
        lambda m: '<div class="proof-box" data-title="Proof">\n\n'
...
print(
    """
    This program will convert your tex file into a functioning post written in markdown/html.
...
try:
    ...
    tex_filename = input(
        'What is the filename of the tex file? (ex. "An-aperiodic-monotile-Finalpaper")\t'
    )
    ...
    try:
        ...
        title_input = input(
            'What is the title of your paper? (ex. "An aperiodic monotile")\t'
        ).title()
        ...
        try:
            ...
            post_foldername = (
                f'{datetime.now().strftime("%Y-%m-%d")}---{format_title(title_input)}'
            )
